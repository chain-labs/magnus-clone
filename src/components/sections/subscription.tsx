...
export default function Subscription() {
  ...
  return (
    <section
      ...
      id={subs.id || "plans"}
    >
      <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 text-center mb-4 sm:mb-10 px-4 leading-tight">
        {subs.title}
      </h1>
          ...
          <div className="flex items-center justify-start sm:justify-center border-b border-gray-200 w-max sm:w-auto mx-auto">
            {subs.billingCycles?.map((cycle) => {
              ...
              return (
                <button
                  key={cycle.key}
                  className={`whitespace-nowrap py-2 px-3 sm:px-4 text-sm sm:text-lg font-semibold border-b-2 transition-colors ${
                    active
      ...
      <div className="container mx-auto px-4 sm:px-20 pt-10 md:pt-14 lg:pt-14">
        <div className="flex flex-wrap justify-center gap-6">
          <div className="relative w-full sm:w-[350px] max-w-full font-poppins rounded-xl bg-white border-[1px] border-blue-200 shadow-[0px_4px_12px_0px_rgba(59,130,246,0.1)] py-4 cursor-pointer overflow-hidden flex flex-col">
              ...
              <div className="overflow-hidden rounded-lg w-full">
                ...
                <div className="flex items-baseline">
                  <h2 className="text-3xl font-bold text-black">
                    {plan.currencySymbol}
                    {price}
                    <span className="text-gray-600 text-sm"> {plan.priceSuffix}</span>
                  </h2>
                </div>
              </div>
            </div>
